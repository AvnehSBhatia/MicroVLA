...
target's start \emph{position} is constant to float64 rounding (two values,
1 ULP apart) in 6 of 10 tasks, its \emph{orientation} is bit-identical across
all 50 states in \textbf{all ten}, and the ten tasks between them use just
\textbf{two} table positions, 22\,cm apart. A policy that memorizes two coordinates can present as
...
exactly, reading the shipped init files directly: in 6 of 10 tasks the target's
start \emph{position} takes exactly two distinct float64 values, one ULP apart
(${\sim}3\times10^{-17}$\,m) on a single axis, and varies by 0.25--0.58\,cm
(std) in the other four; the target's \emph{orientation} is bit-identical
across all 50 states in \textbf{all ten} tasks. \emph{Across} tasks the
structure is tighter still --- the ten targets occupy just \textbf{two} table
positions, 22.0\,cm apart, five tasks each, agreeing to within 0.95\,mm (max
pairwise euclidean) inside a group. A policy that memorizes two
