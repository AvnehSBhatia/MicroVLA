...
\textbf{state-generation defect}: the benchmark's own task files specify the
...
LIBERO-Object tasks, changing \emph{only} the two state-array columns holding
the target's $x$ and $y$ --- every other object, every joint velocity and the
...
$16/100$ and the released head $8/100$.
...
Both measurements need the same two things, and neither needs a model: a
