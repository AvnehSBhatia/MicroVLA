...
lookup-admissible. Every LIBERO-Object task has $D \le 2.34$\,cm; the smallest
diameter among the other suites' $28$ tasks is $2.98$\,cm. \emph{Every} $\delta$
in that gap returns the same verdict, $10/10$ against $0/28$, so no choice of
threshold is doing the work. Six of LIBERO-Object's ten targets take just two
\texttt{float64} values, one unit in the last place apart, across all fifty
shipped states.
...
Run on \emph{all ten} tasks it does better than that. \textbf{The blind
constant scores $16/100$ and our released policy $8/100$}; paired over the $100$
matched trials it wins $10$ and loses $2$, exact McNemar $p = 0.039$. On
\texttt{bbq\_sauce} --- the task carrying the suite's \emph{largest} placement
diameter --- it takes $10/10$ where the trained policy takes $0/10$. A suite on
which a task-indexed constant beats a trained vision-language policy is not
measuring the capability its name implies; that our own policy is the one being
beaten is the other half of the result.
...
\addlinespace
3 & \dots\ and run on the whole suite (\S\ref{sec:fullsuite}) &
constant $\mathbf{16/100}$ vs.\ trained policy $8/100$; paired $p = 0.039$ \\
\addlinespace
...
re-derives all $74$ load-bearing values in this paper from those files and
...
Extracting a target's position from a shipped state is not quite mechanical ---
a naive fixed-column layout is wrong for every suite containing an articulated
fixture --- so we compile each task's model and read \texttt{jnt\_qposadr}.
...
Figure~\ref{fig:ladder}B lists the five arms and what each may read. Two need a
word beyond that. \textbf{random} draws from the observed extent of the scene's
own objects, re-drawn per episode --- the controller's floor, what the machinery
scores when the goal carries no information about where the object is.
\textbf{reset-oracle} reads the true position \emph{once} at reset and holds it;
on a task with $D = 0$ that is numerically a per-task constant, but it still
reads the simulator, so it is \textbf{not} Proposition~\ref{prop:lookup}'s
policy. The arm that is appears below.
...
\caption{\textbf{What the blind constant's failures are made of.}
\textbf{A} On task $0$, which trials fail is predicted by how far that shipped
state's basket sits from the single constant the blind arm was given: the four
failures are the four largest displacements, one rank inversion, exact
Mann-Whitney $p = 0.019$. \textbf{B} On task $3$ the same quantity runs higher
--- four of ten states exceed task $0$'s largest \emph{success} --- and every
trial succeeds. So A1 holds at a radius, that radius explains which trials fail
within a task, and it is not the same radius across tasks. Artifacts:
\texttt{results/blind\_failure\_attribution.json},
\texttt{results/suite\_forensics\_joints.json}.}
\label{fig:subgoal}
\end{figure*}

\paragraph{A prediction we made, and the task that refuted it.} The obvious
reading of the above is that the diameter decides each sub-goal: the soup is
pinned at $D = 0$ so the reach works; the basket is randomised at
$D = 3.37$\,cm so the place fails; $6/10$ is the composition. We wrote that
down. Then \texttt{bbq\_sauce} returned $10/10$ with a \emph{larger} target
diameter ($2.34$\,cm, the suite's largest) \emph{and} a larger basket diameter
($3.93$\,cm, also the suite's largest). A per-task diameter does not decide a
per-task outcome, and we retract that reading.

What replaces it is measured per trial rather than argued per task
(Figure~\ref{fig:subgoal}). Within task $0$, \emph{which} trials fail is
predicted by how far that state's basket sits from the constant: the four
failures are the four largest displacements, one rank inversion, exact
Mann-Whitney $p = 0.019$. Across tasks the radius differs --- task $3$ succeeds
at displacements up to $1.91$\,cm, above task $0$'s smallest failure at
$1.41$\,cm.

\textbf{A1 holds at a radius; the radius explains which trials fail inside a
task; and the radius is not the same number across tasks.} That is a sharper
and less comfortable statement than the one we set out to make, and it is the
one the data supports. Per-trial outcomes and
the paired tests are in \texttt{results/blind\_cells.json}.

\paragraph{The whole suite, not one task.} Every cell above is task $0$, and so
is nearly every confirmatory cell in this paper. We therefore ran the blind arm
on \emph{all ten} LIBERO-Object tasks, $n{=}10$ each, every trial recorded
(Figure~\ref{fig:suite}). It scores $\mathbf{16/100}$ $[0.10, 0.24]$: $6/10$ on
alphabet soup, $\mathbf{10/10}$ on \texttt{bbq\_sauce}, and $0/10$ on the other
eight.

The $10/10$ is worth pausing on. It is the task with the suite's
\emph{largest} placement diameter --- $2.34$\,cm, the value that sets the
admissibility threshold in Table~\ref{tab:suites} --- so admissibility is not
merely a statement about the most degenerate tasks; a constant saturates the
suite's hardest one. And a policy with no camera, no language and no learned
parameter takes it outright.
...
\paragraph{A single-variable ablation, obtained by accident.} The reset-oracle
and blind arms turn out to differ in \emph{one} quantity. Their grasp goals
coincide, because task $0$'s shipped target pose is constant across all fifty
states on all three axes (per-axis standard deviation $0.00$\,cm), so reading it
from the simulator at reset and reading it from the files return the same
point --- up to whatever the simulator does between loading a state and the
first observation, and the blind arm's $2$\,mm approach in all ten trials bounds
that. What differs is the container: the reset-oracle is handed the true basket,
the blind arm a constant one.

\textbf{$10/10 \to 6/10$, and the only thing that changed is the one quantity
this suite randomises.} Four discordant pairs, all favouring the true
container. We did not design this contrast; it fell out of a suite that pins
one sub-goal and randomises the other.
...
\caption{\textbf{The whole suite, lookup against learning.} All ten
LIBERO-Object tasks, $n{=}10$ each, every trial recorded; both policies drive
the identical controller and differ only in where the goal comes from.
\textbf{A} The trained head wins task $0$ ($8$ vs $6$); the blind constant wins
task $3$ outright ($10$ vs $0$), the task carrying the suite's largest
placement diameter; on the remaining eight both score zero. \textbf{B} Suite
totals with Wilson intervals. Artifacts: \texttt{results/suite\_cells.json},
\texttt{results/blind\_cells.json}, \texttt{results/pod\_cells.json}.}
\label{fig:suite}
\end{figure*}

Every confirmatory cell so far is one task. This section is all ten, for both
policies, at $n{=}10$ each, every trial recorded --- and it is where the two
halves of this paper's honesty meet.

\begin{table}[h]\centering\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llll}
\toprule
policy & reads & suite score & Wilson 95\% \\
\midrule
\textbf{blind} & shipped files & $\mathbf{16/100}$ & $[0.10, 0.24]$ \\
released head & camera $+$ language & $8/100$ & $[0.04, 0.15]$ \\
\bottomrule
\end{tabular}
\caption{All ten LIBERO-Object tasks. \textbf{A policy that never sees the
episode outscores the one we trained, by a factor of two.} Paired over the
$100$ matched (task, trial) pairs --- same states, same controller, same
success predicate --- blind wins $10$ and loses $2$, exact McNemar
$p = 0.039$. On \texttt{bbq\_sauce} alone the split is $10$--$0$,
$p = 0.002$. Artifacts: \texttt{results/suite\_summary.json},
\texttt{results/suite\_cells.json}.}
\label{tab:fullsuite}
\end{table}

\paragraph{What this says about the benchmark.} A suite on which a task-indexed
constant beats a trained vision-language policy, significantly and on matched
trials, is not measuring the capability its name implies. This is
Corollary~\ref{cor:sep} at suite scale: not that the score is a weak instrument
for separating lookup from perception, but that on this suite it points the
wrong way.

\paragraph{What it says about us.} \textbf{Eight successes in a hundred trials,
every one on the single object the head was trained for.} Our artifact is a
one-object solution that a suite-level score would have reported as a policy,
and the number a reader should carry away is not $0.700$ --- it is $0.700$ on
task $0$, with $0.000$ next to it \emph{nine} times.

Both readings are load-bearing and neither rescues the other. The benchmark is
degenerate \emph{and} our policy is narrow; the first is why the second was
easy to miss.

\paragraph{And it bounds the audit in advance.} Whatever
\S\ref{sec:audit}--\S\ref{sec:repairs} bought, it was not a general policy. The
repairs made one object work and made the failures on the others legible --- the
drift case study (Table~\ref{tab:audit}, last row) is one of those objects,
lifted from $3/50$ to $22/50$ by DAgger on the policy's own viewpoints. We put
this section before the audit rather than after it so that no reader reaches
the repairs holding a larger idea of the artifact than the artifact supports.
...
predicts. Across the suite that policy --- which has no camera, no language and
no learned parameter, and never sees the episode --- \textbf{outscores the one
we trained}, $16/100$ against $8/100$, $p = 0.039$ on matched trials.

We report that as a fact about the benchmark and a fact about our artifact at
once, because it is both, and because a suite-level score would have let us
report only the flattering half.
...
\emph{(head, stack)} --- and report, as we have, the claims that did not survive
being powered up, including one of our own from four pages earlier.
