...
\paragraph{One task is not the claim.} Every cell above is task $0$. We ran the
blind arm on all ten, and \S\ref{sec:fullsuite} reports the result together with
the same ten tasks for the trained head. One number belongs here, because it
bears on admissibility rather than on either policy: the blind constant takes
$\mathbf{10/10}$ on \texttt{bbq\_sauce}, the task carrying the suite's
\emph{largest} placement diameter --- the $2.34$\,cm that sets the threshold in
Table~\ref{tab:suites}. \textbf{A constant saturates the suite's hardest task,
not merely its most degenerate ones.}
