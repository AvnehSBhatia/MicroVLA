...
declare, and land on the same spread as the sibling suite that never needed
repairing. The candidate suite is in \texttt{results/resampled\_init}. We
state its limit plainly: positions are drawn from the region the benchmark
itself declares valid, so they should be admissible by construction, but we
have not run them through a simulator to confirm no interpenetration. It is a
candidate repair, and the manifest says so.}
\label{tab:repaired}
\end{table}
