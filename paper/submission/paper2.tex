...
It is not confined to one task. On \texttt{bbq\_sauce} --- the task carrying
the suite's \emph{largest} placement diameter --- the same blind constant scores
$\mathbf{10/10}$ where our released policy scores $0/10$. Where blind does fail
it fails legibly: on task $0$ the four failing trials are the four shipped
states whose basket sits furthest from the constant it was handed (exact
Mann-Whitney $p = 0.019$), which is the paper's tolerance assumption being
