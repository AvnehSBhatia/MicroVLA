...
target from. \textbf{LIBERO-Long exercises its declared $5 \times 5$\,cm box on
every task ($96.7$--$99.8\%$ of it). LIBERO-Object, given the identical box by
the same generator, uses $0$--$38.9\%$ --- and on six of ten tasks all fifty
shipped states are a single point.} The two suites do not overlap, so this is
not a property of the generator; it is a state-generation defect in one
release, and the benchmark's own task files already specify the fix.
...
point --- the box's exact centre. \textbf{The generator randomises; the
...
\begin{tabular}{lrrr}
\toprule
suite & declared & shipped spread & fraction used \\
\midrule
\textbf{Object} & $5 \times 5$\,cm & $0.00$--$1.94$\,cm & $\mathbf{0.0}$--$\mathbf{38.9\%}$ \\
Long & $5 \times 5$\,cm & $4.83$--$4.99$\,cm & $96.7$--$99.8\%$ \\
\bottomrule
\end{tabular}
\caption{\textbf{Two suites of the same benchmark, the same generator, the
identical declared region --- and no overlap in what they ship.} LIBERO-Long
exercises its $5 \times 5$\,cm box essentially fully on all ten tasks;
LIBERO-Object uses a tenth of it on average and \emph{none of it at all} on six
of ten, where all fifty states are one point. This is arithmetic on two shipped
files: no $\delta$, no policy, no simulator, and Long is the internal control
that rules out a generator-wide explanation. LIBERO-Spatial and Goal declare
regions their shipped states exceed, so we read our region parsing as a lower
bound there and rest the comparison on the two suites that declare the same
box. Artifact: \texttt{results/declared\_vs\_shipped.json}.}
\label{tab:declared}
\end{table}
