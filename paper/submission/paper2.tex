...
comparison supports nothing}, which is the paper's second contribution.
...
We cannot separate a remaining defect in our harness from a checkpoint that
does not transfer to a differently-built LIBERO, and the reproduction gap we
hit is a known community issue rather than a finding of ours. The harness ships
...
before the instrument was trusted. Deployed binding accuracy is reported as
\textbf{unmeasured}; the six gates are in the appendix.
...
\S\ref{sec:adm}'s verdict holds over a window, and the window is narrow. We
report the whole curve because the previous version of this paper claimed the
threshold was not load-bearing, and on the corrected quantity that claim is only
true locally.
...
\toprule
$\delta$ (cm) & Object & Spatial & Goal & Long & elsewhere \\
\midrule
$0.5$ & 6/10 & 0/10 & 2/10 & 0/10 & 2/30 \\
$1.0$ & 9/10 & 0/10 & 2/10 & 0/10 & 2/30 \\
\textbf{1.17} & \textbf{10/10} & 0/10 & 2/10 & 0/10 & \textbf{2/30} \\
\textbf{1.40} & \textbf{10/10} & 0/10 & 2/10 & 0/10 & \textbf{2/30} \\
\textbf{1.49} & \textbf{10/10} & 0/10 & 2/10 & 0/10 & \textbf{2/30} \\
$2.0$ & 10/10 & 8/10 & 9/10 & 0/10 & 17/30 \\
$2.5$ & 10/10 & 9/10 & 10/10 & 0/10 & 19/30 \\
$4.0$ & 10/10 & 9/10 & 10/10 & 10/10 & 29/30 \\
\bottomrule
\end{tabular}
\caption{Tasks whose identity-bearing object a single constant serves, as a
function of the tolerance. \textbf{The separation exists on
$[1.17, 1.49]$\,cm and essentially nowhere above it}: by $\delta = 2$\,cm most
of LIBERO-Spatial and LIBERO-Goal are admissible too. Our measured radius
($\approx 1.41$\,cm, \S\ref{sec:refute}) sits inside the window, but the window
is $0.32$\,cm wide and the measurement that places it there is a single
$n{=}10$ cell with the caveats given in \S\ref{sec:refute}. A reader should
treat the verdict as conditional on that measurement, and a $2$\,cm tolerance
would license a much weaker claim. Artifact:
\texttt{results/admissibility.json}.}
\label{tab:deltasens}
\end{table}
