...
addendum carries the multi-object campaign: a second object crossed and twice
confirmed on never-scored seeds, a third measured, mechanism-located at role
binding, and \emph{not} crossed---with a pre-registered prediction of that
crossing recorded and falsified.
\end{abstract}
...
confirmations (5/10 + 3/10; 5/10 + 4/10 on a three-object head). A third
object (cream cheese) does not cross, and the mechanism is located rather than
asserted: its goals are accurate (1.58\,cm on-manifold, vs.\ 1.34 and 1.06 for
the objects that work) while its \emph{role binding} fails.
...
We pre-registered, and record as falsified, the prediction that 0.82 per-tick
identity accuracy would yield 0.914 median-of-3 latch correctness and hence
cream $>0$ under bank binding; the cell returned 0/10. The error is
