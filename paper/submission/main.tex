...
crossed and twice confirmed on never-scored seeds, two \emph{not} crossed and
---after a proposed mechanism was tested and retracted---not explained. Three
pre-registered predictions were recorded and falsified, and five instruments
built to quantify the boundary were all abandoned at their own calibration
gates. The retracted mechanism is instructive: every object, including the two
that succeed, is detected by the same generic shape prompt rather than by its
name, so ``the detector cannot name it'' cannot be why the other two fail.
\end{abstract}
...
generic prompt, the difference must lie in the scene rather than the prompt
(how many shape-alike distractors compete for it): an untested hypothesis, not
a finding.
