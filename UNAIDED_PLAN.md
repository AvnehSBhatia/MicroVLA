...
| B. Teacher dataset | Roll winning handeye on train inits; store frames+proprio+boxes+actions via shard pipeline (BudgetGuard) | N episodes cream (+ soup) | recorder being built (`eval/record_teacher.py`) |
| C. Distill stage B | From `rec_fix`, BC/DAgger on teacher actions; grasp+place weighted; freeze fusion/TRM first | val mimics teacher; **unaided cream succ > 0, n≥10** | — |
| D. Internalize offset+z | eef-target / z-at-close aux losses so distillation isn't the only geometry carrier | ablate teacher, keep aux; succ ≠ 0 | — |
